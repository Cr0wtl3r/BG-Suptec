<script lang="ts">
  ...
  // MUDANÇA: A struct agora reflete a nova struct do Go
  type TecladoInfo = {
    id: string;
    nome: string;
    tagIdioma: string;
  ...
  let idLayoutAtivo = "";
  let layoutSelecionado: string | undefined = undefined; // Continua sendo o ID técnico
  ...
  async function aplicarLayout() {
    ...
    // MUDANÇA: Encontra o objeto completo do layout para pegar a tagIdioma
...
<FeatureContainer titulo="Alterar Layout do Teclado">
  <div class="space-y-6">
    ...
    <div>
      <label for="layout-select" class="block font-bold"
