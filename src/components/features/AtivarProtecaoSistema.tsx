...
interface AtivarProtecaoSistemaProps {
  onVoltar: () => void;
}

function AtivarProtecaoSistema({ onVoltar }: AtivarProtecaoSistemaProps) {
  ...
  useLogEvent(EVENTOS.logAtivarProtecao, adicionarLog);
  ...
  async function iniciar() {
    ...
    try {
      await invoke("ativar_protecao_sistema");
    } catch (err) {
  ...
  return (
    <FeatureContainer titulo="Ativar Proteção do Sistema">
      <div className="flex min-h-0 flex-grow flex-col">
        <div className="mx-auto w-full max-w-xl flex-shrink-0 pr-2">
          <p className="mb-6 mt-0 text-center opacity-90">
            Esta função ativa a Restauração do Sistema para a unidade C: e configura o uso de
            disco para 5%. Isso permite reverter o computador para um estado anterior em caso de
            problemas. Requer privilégios de Administrador.
          </p>
          <button
            ...
            disabled={emExecucao}
          >
            {emExecucao ? "Ativando..." : "Ativar Proteção"}
          </button>
        </div>
...
export default AtivarProtecaoSistema;
